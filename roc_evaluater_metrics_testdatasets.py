...
class Evaluate(object):
    ...
    def f1score(self):            
        ...
        TP, TN, FP, FN, fpr, tpr = Evaluate.roc(self)
        try:
            f1scorev = 2*(float(prec * rec)/float(prec + rec))
        except ZeroDivisionError:
            f1scorev = 0
        '''
        with open('./evaluation_results/testdatasets_evaluation_metrics/segnet_roc_evaluationmetrics_test.txt', 'a')as f:
        ...
        with open('./evaluation_results/testdatasets_evaluation_metrics/segnet_roc_evaluationmetrics_test.csv','a', newline = '') as f:
            # create the csv writer
            writer = csv.writer(f)
            data = [image_name, TP, TN, FP, FN,fpr, tpr, prec, rec, f1scorev]
...
if __name__ == "__main__":
    ...
    header = ['image_name', 'TP', 'TN', 'FP', 'FN','FPR','TPR','Precion','Recall','F1score']
        ...
        # open the file in the write mode
    with open('./evaluation_results/testdatasets_evaluation_metrics/segnet_roc_evaluationmetrics_test.csv','w', newline = '') as f:
            # create the csv writer
            writer = csv.writer(f)
    ...
    f = open('./evaluation_results/testdatasets_evaluation_metrics/segnet_mean_roc_evaluation_metrics_test.txt', 'w')
